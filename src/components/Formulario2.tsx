...
export default function Formulario(props: FormularioProps){
    ...
    const handlePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSenhaValida(newPassword?.length >= 8 && /[A-Z]/.test(newPassword) && /\d/.test(newPassword));
      };
    ...
    return (
        <div className="text-black">
            ...
            <Entrada texto="Senha" valor={senha} valorMudou={handlePasswordChange} tipo="password" placeholder="Digite sua senha"/>
            <Entrada texto="Senha (novamente)" valor={senhaConfirmacao} valorMudou={(valor) => {
                setSenhaConfirmacao(valor);
                setSenhaValida(valor === senha);
            }} tipo="password" placeholder="Digite sua senha novamente"/>
            {!senhaValida && <p className="text-red-500 text-sm">A senha deve ter pelo menos 8 caracteres, uma letra maiúscula e um número.</p>}
